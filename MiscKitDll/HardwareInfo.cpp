...
std::vector<CString> CHardwareInfo::GetMacAdd(BOOL bWireless, BOOL bBluetooth, BOOL bVirtual)
{
	std::vector<CString> vMacs;

	DWORD dwMacAdd = 0;
	IP_ADAPTER_INFO AdapterInfo[32];				// Allocate information
	// for up to 16 NICs
	...
	while (pAdapterInfo)
	{
		std::wregex pat(_T(".*[Ww][Ii][Rr][Ee][Ll][Ee][Ss][Ss].*"));
		const CString strTmp = A2W(pAdapterInfo->Description);

		if ((71 == pAdapterInfo->Type || true == regex_match(strTmp.GetString(), pat)) && FALSE == bWireless)
		{
			pAdapterInfo = pAdapterInfo->Next;
			continue;
		}

		pat = (_T(".*[Bb][Ll][Uu][Ee][Tt][Oo][Oo][Tt][Hh].*"));

		if ((71 == pAdapterInfo->Type || true == regex_match(strTmp.GetString(), pat)) && FALSE == bBluetooth)
		{
			pAdapterInfo = pAdapterInfo->Next;
			continue;
		}

		pat = (_T(".*[Vv][Ii][Rr][Tt][Uu][Aa][Ll].*"));

		if (true == regex_match(strTmp.GetString(), pat) && FALSE == bVirtual)
		{
			pAdapterInfo = pAdapterInfo->Next;
			continue;
		}
